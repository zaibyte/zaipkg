...
func TestMain(m *testing.M) {
	...
	if xtest.IsPropEnabled() { // Only run smhasher testing when properties testing is enabled.
		os.Exit(m.Run())
	}
}
...
type hashSet struct {
	m map[uint32]struct{} // set of hashes added
	n int                 // number of hashes added
}

func newHashSet() *hashSet {
	return &hashSet{make(map[uint32]struct{}), 0}
}
func (s *hashSet) add(h uint32) {
	s.m[h] = struct{}{}
	s.n++
}
...
func (s *hashSet) check(t *testing.T) {
	...
	t.Logf("number of collisions: got=%d mean=%f stddev=%f", collisions, expected, stddev)

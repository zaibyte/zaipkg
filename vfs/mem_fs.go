...
func (y *MemFS) Open(fullname string, opts ...lvfs.OpenOption) (File, error) {
...
func (f *memFile) WriteAt(p []byte, off int64) (int, error) {
	if !f.write {
		return 0, errors.New("pebble/vfs: file was not opened for writing")
	}
	if f.n.isDir {
		return 0, errors.New("pebble/vfs: cannot read a directory")
	}
	...
	if off+int64(len(p)) <= int64(len(f.n.mu.data)) {
		return copy(f.n.mu.data[off:off+int64(len(p))], p), nil
	} else {
